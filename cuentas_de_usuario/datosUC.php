...
<body>
    ...
    if (mysqli_num_rows($resultado) > 0) {
        ...
        if (mysqli_query($conexion, $unirrr)) {
            header('Location:/1r Sprint-FMSDigital/Maquetacion/Estudiante/clase_de_alumno.php');
        } else {
            echo "Error al unirse a la clase: " . mysqli_error($conexion);
        }
    } else {
        // Si no encontró el código, le avisa y lo regresa al panel
        echo "<script>alert('El código ingresado no existe'); window.location.href = '/1r Sprint-FMSDigital/Maquetacion/Estudiante/panel_de_estudiante.php';</script>";
    }
?>
</body>
</html>